...
public class BagIvan implements Bag {
	/**
	 * Количество ячеек с информацией о члене множества.
	 * <ul>
	 * <li>0 - член множества;</li>
	 * <li>1 - количество экземпляров во множестве.</li>
	 * </ul>
	 */
	private final int AMO_INFORMATION_CELLS = 2;

	/** Хранилище членов множества и информации о их количестве в этом множестве. */
	...
	public void remove(int val) {
		// При данной реализации возможна утечка оперативной памяти, т.к. информация о членах множества,
		// утративших все экземпляры, остаётся.
		if (bag != null) {
			for (int i = 0; i < bag.length; i++) {
				if (bag[i][0] == val) {
					if ((bag[i][1] > 0)) {
